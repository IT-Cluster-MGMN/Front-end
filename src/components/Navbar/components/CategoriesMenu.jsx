...
const CategoriesMenu = () => {
  const CATEGORIES_MENU_TEXT =
    "font-sans font-semibold text-black text-[10px] no-underline";
  const CATEGORIES_MENU_BUTTONS =
    "flex items-center justify-start w-full gap-2 px-5 py-2 border-none bg-zinc-300 m-[5px] hover:bg-zinc-400";
      ...
      <div className="flex flex-row absolute justify-self-center left-[3%] z-10 w-[94%] max-h-[300px]">
        ...
        <div className="flex flex-col items-start w-[25%] bg-zinc-300 rounded-bl-[10px] py-4 overflow-y-auto">
          {categories.map((value) => (
            ...
            <button
              className="flex w-full gap-2 px-5 my-2 border-none bg-inherit hover:underline hover:cursor-pointer"
          ...
          {category["subcategories"].map((subcategory) => (
            <div className="flex flex-col px-3 py-2">
              ...
              {subcategory["subsubcategory"].map((subsubcategory) => (
                <Link
                  className="font-sans text-[13px] text-black no-underline hover:underline"
