...
const Login = () => {
    ...
    const handleSubmit = (e) => {
        ...
        try {
            ...
            customRequest.post(API_ENDPOINT, toPost).then((res) => {
            ...
            {success ? (
                ...
                drop-shadow-xl"
                    >
                        ...
                        <p
                            ref={errRef}
                            className={`text-red-500 w-40 inline-block bg-white text-center rounded-2xl border-white ${errMsg ? "errmsg" : "offscreen"}`}
                            aria-live="assertive"
                        >
                            {errMsg}
                        </p>
                        ...
                        <div className="relative w-40 h-40">
                            <label className="absolute flex items-center justify-center w-full h-full bg-white rounded-full shadow-inset text-align-center">
                                <img
                                    className="absolute object-cover rounded-full top-2 left-2 w-36 h-36"
                ...
                mt-[30px]`}
                        ...
                        {/* Ел пошта  */}
                        <label
                            className={`${FONT_CLASS} text-white mt-[20px] text-left`}
                            htmlFor="username"
                        >
                ...
                w-[94%]`}
                        ...
                        <p className="flex justify-between sign2 text-white mt-[50px] w-[90%]">
                            <Link
                                className={`${FONT_CLASS} text-white no-underline visited:text-white`}
                                to="/resetpassword"
                            >
                                Забули пароль?
                            </Link>
