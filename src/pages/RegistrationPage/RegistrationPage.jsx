...
const RegistrationPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const customRequest = axios.create({ withCredentials: true });
    ...
    try {
      ...
      customRequest
        .post(SECURITY_API, createSecurity)
        .then(() => {
          customRequest
            .post(PROFILE_API, createPersonal)
            .then(() => {
      ...
      {success ? (
        ...
        <section className="flex items-center justify-center min-h-screen">
          ...
          >
            ...
            <label
              htmlFor="username"
              className={`${FONT_CLASS} flex flex-row items-center text-white mt-[20px] text-left`}
            >
              {email && (
                <>
                  {validEmail ? (
            ...
            <p
              ...
              className={
                !emailFocus || validEmail
                  ? "w-0 overflow-hidden text-transparent h-0 transition-transform delay-150 duration-300"
            ...
            <div className="w-[95%]">
              <button
                ...
                disabled={
                  validEmail && validPassword && validMatch ? false : true
                }
              >
                Sign Up
              </button>
            ...
            <p className="flex justify-between sign2 text-white mt-[50px] w-[90%]">
              <Link
                className={`${FONT_CLASS} no-underline visited:text-white text-[15px]`}
                to="/login"
              >
                Sign in
              </Link>
